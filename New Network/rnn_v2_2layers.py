...
path = directory + "\log\layer_2_model_{}.ckpt".format(now)
print(path)
n_steps   = 25
n_inputs  = 127
n_neurons = 150
...
layers = [tf.contrib.rnn.BasicRNNCell(num_units=n_neurons) for layers in range(n_layers)]
multi_layer_cell = tf.contrib.rnn.MultiRNNCell(layers)
#outputs, states = tf.nn.dynamic_rnn(cell, X, dtype=tf.float32)
outputs, states = tf.nn.dynamic_rnn(multi_layer_cell, X, dtype = tf.float32)

logits_before_bn = tf.layers.dense(states[(n_layers-1)], n_outputs, name='outputs')
